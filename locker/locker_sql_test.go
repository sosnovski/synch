...
func (s *LockerSQLTestSuite) TestTryLockDo() {
	t := s.T()
...
func (s *LockerSQLTestSuite) TestTryLockDoWithError() {
	t := s.T()
...
func (s *LockerSQLTestSuite) TestTryLockDoWithCloseError() {
	t := s.T()
	...
	require.ErrorContains(t, err, "delete lock: delete from ")
}
